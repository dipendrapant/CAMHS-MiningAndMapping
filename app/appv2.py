...
def axis_summary(tab, data_path, prefix):
    st.header(f"Patient Profile / Pasientprofildistribusjoner")
    # write subheader
    st.subheader(
        "Summary is based on total unique patients in the axis/ Oppsummering er basert på unike pasienter i aksen"
    )
    ...
    with st.expander("Filter Options / Filteralternativer", expanded=False):
        ...
        if apply_icd:
            icd_opts = [""] + [
                f"{code}: {mapped_icd_codes.get(code,'Unknown')}"
                for code in sorted(mapped_icd_codes)
            ]
            sel = st.selectbox(
            ...
            icd_in = sel.split(":", 1)[0] if sel else ""
        ...
        atc_in = "All"
        if apply_atc:
            atc_opts = ["All"] + [f"{c}: {n}" for c, n in sorted(atc_map.items())]
            sel2 = st.selectbox(
                "Select ATC Code / Velg ATC-kode:",
                atc_opts,
                key=f"{prefix}_atc_sbox",
            )
            atc_in = sel2.split(":", 1)[0] if sel2 and sel2 != "All" else "All"
    ...
    for field, title in [
        ("Mother Relationship", "Mother Relationship Distribution (Unique Patients)"),
        ("Father Relationship", "Father Relationship Distribution (Unique Patients)"),
        ("Mother Ethnicity", "Mother Ethnicity Distribution (Unique Patients)"),
        ("Father Ethnicity", "Father Ethnicity Distribution (Unique Patients)"),
        ("Home Language", "Home Language Distribution (Unique Patients)"),
    ]:
        if field in sel:
...
def axis_analysis(tab, load_func, data_path, prefix):
    ...
    with st.expander("Filter Options", expanded=False):
        ...
        if apply_icd:
            ...
            sel_icd = st.selectbox(
                "Select ICD Code", icd_opts, key=f"{prefix}_icd_sbox"
            )
            icd1 = sel_icd.split(":", 1)[0] if sel_icd else ""
            ...
            if apply_atc:
                ...
                atc1 = (
                    sel_atc.split(":", 1)[0] if sel_atc and sel_atc != "All" else "All"
                )
